...
void gerar_pbm(const char *sequencia, int pixels_por_area, int altura, int espaco_lateral, const char *nome_arquivo) {
    int largura_codigo = strlen(sequencia) * pixels_por_area; // Largura total das barras
    int largura_total = largura_codigo + 2 * espaco_lateral; // Largura total com espaçamento
    int altura_total = altura; // Altura da imagem
    ...
    for (int y = 0; y < altura_total; y++) {
        for (int x = 0; x < largura_total; x++) {
            if (x < espaco_lateral || x >= largura_total - espaco_lateral) {
                // Espaço lateral (pixels brancos)
                fprintf(arquivo, "0");
            } else {
                // Determinar a posição na sequência
                int posicao_codigo = (x - espaco_lateral) / pixels_por_area; 
...
int main(int argc, char *argv[]) {
    ...
    // Testando para saber se o dígito verificador é válido ou não
    if (!validar_digito_verificador(digito_id)) {
